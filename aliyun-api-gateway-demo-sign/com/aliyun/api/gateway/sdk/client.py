...
class DefaultClient:
    ...
    def build_headers(self, request=None):
        ...
        if (header_params.get(constant.X_CA_STAGE)):
            headers[constant.X_CA_STAGE] = header_params.get(constant.X_CA_STAGE)
        ...
        if constant.POST == request.get_method() and constant.CONTENT_TYPE_STREAM == request.get_content_type():
            ...
            str_to_sign = signature_composer.build_sign_str(uri=request.get_url(), method=request.get_method(),
                                                            headers=headers, sign_with_body=request.get_sign_with_body())
        else:
            str_to_sign = signature_composer.build_sign_str(uri=request.get_url(), method=request.get_method(),
                                                            headers=headers, body=body, sign_with_body=request.get_sign_with_body())
